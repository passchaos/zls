...
pub const Handle = struct {
    ...
    fn parseTree(allocator: std.mem.Allocator, new_text: [:0]const u8, mode: Ast.Mode) error{OutOfMemory}!Ast {
        ...
        try compactMultiArrayList(allocator, &nodes);
        tree.nodes = nodes.slice();

        // remove unused capacity
        var tokens = tree.tokens.toMultiArrayList();
        try compactMultiArrayList(allocator, &tokens);
        tree.tokens = tokens.slice();
        return tree;
    }

    fn compactMultiArrayList(allocator: std.mem.Allocator, list: anytype) error{OutOfMemory}!void {
...
test "AST multi-array compaction preserves fields" {
    ...
    try Handle.compactMultiArrayList(allocator, &list);
...
test "AST multi-array compaction falls back safely" {
    const Item = struct { large: u64, small: u8, medium: u32 };
    var empty: std.MultiArrayList(Item) = .empty;
    try Handle.compactMultiArrayList(std.testing.allocator, &empty);
    ...
    try std.testing.expectError(error.OutOfMemory, Handle.compactMultiArrayList(failing.allocator(), &list));
    ...
    try Handle.compactMultiArrayList(fallback.allocator(), &list);
    try std.testing.expectEqual(list.len, list.capacity);
