...
test "function-valued field does not consume an implicit self parameter" {
    try testInlayHints(
...
test "comptime return types - HashMap" {
    try testInlayHints(
        ...
        \\const boolMap<HashMap(i32,bool,AutoContext(i32),80)> = std.AutoHashMap(i32, bool).init(allocator);
        \\const u32Map<HashMap(i32,u32,AutoContext(i32),80)> = std.AutoHashMap(i32, u32).init(allocator);
    ...
    try testInlayHints(
        \\const std<type> = @import("std");
        \\const map<HashMap(i32,HashMap(i32,void,AutoContext(i32),80),AutoContext(i32),80)> = std.AutoHashMap(i32, std.AutoHashMap(i32, void)).init(allocator);
        \\const value<?*HashMap(i32,void,AutoContext(i32),80)> = map.getPtr(123);
