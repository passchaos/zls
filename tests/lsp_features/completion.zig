...
test "comptime complementary identities accept structured vectors" {
    try testCompletion(
        \\var runtime_i8: i8 = undefined;
        \\var runtime_bool: bool = undefined;
        \\fn Select() type {
        ...
        \\    });
        \\    const zeros: @Vector(2, i8) = @splat(0);
        \\    const ones: @Vector(2, i8) = @splat(-1);
        \\    return struct {
        \\        integers: [if (@reduce(.And, (integers & ~integers) == zeros) and
        \\            @reduce(.And, (integers | ~integers) == ones) and
        \\            @reduce(.And, (integers ^ ~integers) == ones) and
        \\            @reduce(.And, (integers + ~integers) == ones)) 1 else 99]u8,
        \\        booleans: [if (!@reduce(.Or, booleans & !booleans) and
        \\            @reduce(.And, booleans | !booleans)) 1 else 99]u8,
        \\        executions: [executions]u8,
        \\    };
        \\}
        \\const selected: Select() = undefined;
...
test "comptime complementary comparisons accept structured vectors" {
    try testCompletion(
        \\var runtime_i8: i8 = undefined;
        \\var runtime_bool: bool = undefined;
        \\fn Select() type {
        ...
        \\    });
        \\    return struct {
        \\        integers: [if (!@reduce(.Or, integers == ~integers) and
...
test "generic function with comptime pointer atomics" {
    try testCompletion(
        \\fn Select(comptime T: type) type {
        \\    var first: usize = 2;
        \\    var second: usize = 5;
        \\    var third: usize = 7;
        \\    var pointer: *usize = &first;
        \\    @atomicStore(*usize, &pointer, &second, .release);
        \\    const loaded = @atomicLoad(*usize, &pointer, .acquire);
        \\    const previous = @atomicRmw(*usize, &pointer, .Xchg, &third, .seq_cst);
        \\    const success = @cmpxchgStrong(*usize, &pointer, &third, &first, .seq_cst, .acquire);
        \\    const failure = @cmpxchgWeak(*usize, &pointer, &third, &second, .monotonic, .monotonic);
        \\    const loaded_value = loaded.*;
        \\    const previous_value = previous.*;
        \\    const current_value = pointer.*;
        \\    const succeeded = success == null;
        \\    const observed_value = failure.?.*;
        \\    return struct {
        \\        loaded: [loaded_value]T,
        \\        previous: [previous_value]T,
        \\        current: [current_value]T,
        \\        succeeded: [@intFromBool(succeeded)]u8,
        \\        observed: [observed_value]u8,
        \\    };
        \\}
        \\const selected: Select(u16) = undefined;
        \\const field = selected.<cursor>
    , &.{
        .{ .label = "loaded", .kind = .Field, .detail = "[5]u16" },
        .{ .label = "previous", .kind = .Field, .detail = "[5]u16" },
        .{ .label = "current", .kind = .Field, .detail = "[2]u16" },
        .{ .label = "succeeded", .kind = .Field, .detail = "[1]u8" },
        .{ .label = "observed", .kind = .Field, .detail = "[2]u8" },
    });
}

test "generic function with comptime enum atomics" {
    try testCompletion(
        \\fn Select(comptime T: type) type {
        \\    const State = enum(u8) { first = 2, second = 5, third = 7 };
        \\    var state = State.first;
        \\    @atomicStore(State, &state, .second, .release);
        \\    const loaded = @atomicLoad(State, &state, .acquire);
        \\    const previous = @atomicRmw(State, &state, .Xchg, .third, .seq_cst);
        \\    const success = @cmpxchgStrong(State, &state, .third, .first, .seq_cst, .acquire);
        \\    const failure = @cmpxchgWeak(State, &state, .third, .second, .monotonic, .monotonic);
        \\    const final = state;
        \\    return struct {
        \\        loaded: [@intFromEnum(loaded)]T,
        \\        previous: [@intFromEnum(previous)]T,
        \\        current: [@intFromEnum(final)]T,
        \\        succeeded: [@intFromBool(success == null)]u8,
        \\        observed: [@intFromEnum(failure.?)]u8,
        \\    };
        \\}
        \\const selected: Select(u16) = undefined;
        \\const field = selected.<cursor>
...
test "generic function with comptime atomic array elements and constants" {
    try testCompletion(
        \\const constants = [_]usize{ 11, 13 };
        \\fn Select(comptime T: type) type {
        \\    var values = [_]usize{ 2, 3, 5 };
        \\    const static_loaded = @atomicLoad(usize, &constants[1], .acquire);
        ...
        \\const field = selected.<cursor>
    , &.{
        .{ .label = "static", .kind = .Field, .detail = "[13]u16" },
...
test "zero-parameter type function comptime evaluation" {
    ...
    try testCompletion(
        ...
        \\}
        \\fn Select() type {
        \\    return struct { items: [value(4).*]u8 };
        \\}
        \\const selected: Select() = undefined;
        \\const field = selected.<cursor>
    , &.{.{ .label = "items", .kind = .Field, .detail = "[4]u8" }});
    ...
    try testCompletion(
        ...
        \\}
        \\fn Select() type {
        \\    return struct { items: [value(runtime).*]u8 };
        \\}
        \\const selected: Select() = undefined;
        \\const field = selected.<cursor>
    , &.{.{ .label = "items", .kind = .Field, .detail = "[?]u8" }});
    ...
    try testCompletion(
        \\fn value() *const usize {
        \\    var runtime: usize = 4;
        \\    const pointer: *const usize = &runtime;
        \\    return pointer;
        \\}
        \\fn Select() type {
        \\    return struct { items: [value().*]u8 };
    ...
    try testCompletion(
        ...
        \\    var result: *const @Vector(2, usize) = &low;
        \\    result = &high;
        \\    return result;
        \\}
        \\fn Select() type {
        \\    return struct { items: [values().*[1]]u8 };
        \\}
        \\const selected: Select() = undefined;
        \\const field = selected.<cursor>
    , &.{.{ .label = "items", .kind = .Field, .detail = "[5]u8" }});
    ...
    try testCompletion(
        \\var runtime: @Vector(2, usize) = undefined;
        \\const low: @Vector(2, usize) = .{ 1, 2 };
        \\fn values() *const @Vector(2, usize) {
        \\    var result: *const @Vector(2, usize) = &low;
        \\    result = &runtime;
    ...
    try testCompletion(
        \\const one: usize = 1;
        \\const four: usize = 4;
        \\const low: *const usize = &one;
        \\const high: *const usize = &four;
        \\fn value() *const *const usize {
        \\    var result: *const *const usize = &low;
        \\    result = &high;
        \\    return result;
        \\}
        \\fn Select() type {
    ...
    try testCompletion(
        ...
        \\const one: usize = 1;
        \\const low: *const usize = &one;
        \\const unknown: *const usize = &runtime;
        \\fn value() *const *const usize {
        \\    var result: *const *const usize = &low;
        \\    result = &unknown;
        \\    return result;
    ...
    try testCompletion(
        ...
        \\}
        \\fn callback() *const fn () usize {
        \\    var result: *const fn () usize = &one;
        \\    result = &four;
        \\    return result;
        \\}
        \\fn Select() type {
        \\    return struct { items: [callback()()]u8 };
        \\}
        \\const selected: Select() = undefined;
        \\const field = selected.<cursor>
    , &.{.{ .label = "items", .kind = .Field, .detail = "[4]u8" }});
    ...
    try testCompletion(
        \\var runtime_callback: *const fn () usize = undefined;
        \\fn one() usize {
        \\    return 1;
        \\}
        \\fn callback() *const fn () usize {
        \\    var result: *const fn () usize = &one;
        \\    result = runtime_callback;
        \\    return result;
        \\}
        \\fn Select() type {
        \\    return struct { items: [callback()()]u8 };
    ...
    try testCompletion(
        ...
        \\}
        \\fn Select() type {
        \\    return struct { items: [values().len]u8 };
        \\}
        \\const selected: Select() = undefined;
        \\const field = selected.<cursor>
    , &.{.{ .label = "items", .kind = .Field, .detail = "[?]u8" }});
    ...
    try testCompletion(
        ...
        \\}
        \\fn Select() type {
        \\    return struct { items: [choice().large]u8 };
        \\}
        \\const selected: Select() = undefined;
        \\const field = selected.<cursor>
    , &.{.{ .label = "items", .kind = .Field, .detail = "[4]u8" }});
    ...
    try testCompletion(
        ...
        \\fn values() *const Pair {
        \\    var result: *const Pair = &.{ 1, 2 };
        \\    result = &.{ 4, 5 };
        \\    return result;
        \\}
        \\fn Select() type {
    ...
    try testCompletion(
        ...
        \\}
        \\fn Select() type {
        \\    return switch (selectedError()) {
        \\        error.First => struct { first: u8 },
        \\        error.Second => struct { second: u8 },
        \\    };
        \\}
        \\const selected: Select() = undefined;
        \\const field = selected.<cursor>
    ...
    try testCompletion(
        ...
        \\}
        \\fn Select() type {
        \\    const value = selectedValue() catch 7;
        \\    return struct { items: [value]u8 };
        \\}
        \\const selected: Select() = undefined;
        \\const field = selected.<cursor>
    ...
    try testCompletion(
        \\fn selectedValue() ?usize {
        \\    var value: ?usize = null;
        \\    value = 4;
        \\    return value;
        \\}
        \\fn Select() type {
        \\    return if (selectedValue()) |payload|
        \\        struct { items: [payload]u8 }
        \\    else
        \\        struct { absent: u8 };
        \\}
    ...
    try testCompletion(
        ...
        \\const selected: Select() = undefined;
        \\const field = selected.<cursor>
    , &.{.{ .label = "safe", .kind = .Field, .detail = "u8" }});
    ...
    try testCompletion(
        ...
        \\}
        \\fn Select() type {
        \\    return struct { items: [3:sentinel()]u8 };
        \\}
        \\const selected: Select() = undefined;
        \\const field = selected.<cursor>
...
test "comptime pointer comparisons preserve address identity" {
    ...
    try testCompletion(
        ...
        \\    var distinct: usize = 7;
        \\    if (first != second) distinct = 6;
        \\    var offset_same: usize = 9;
        \\    if (@as([*]const usize, @ptrCast(second)) == pointer + 1) offset_same = 8;
        \\    return struct { value: [second.*]u8, distinct: [distinct]u8, offset_same: [offset_same]u8 };
        \\}
        \\const selected: Select() = undefined;
        \\const field = selected.<cursor>
    , &.{
        .{ .label = "value", .kind = .Field, .detail = "[5]u8" },
        .{ .label = "distinct", .kind = .Field, .detail = "[6]u8" },
        .{ .label = "offset_same", .kind = .Field, .detail = "[8]u8" },
    });
...
test "comptime static sentinel slice boundary" {
    try testCompletion(
        \\const values = [_:9]usize{ 3, 5, 7 };
...
test "comptime static sentinel slice pointer boundary" {
    try testCompletion(
        \\const values = [_:9]usize{ 3, 5, 7 };
        \\const slice: [:9]const usize = &values;
        \\fn Select() type {
        \\    const value = slice.ptr[1];
        \\    return struct { value: [value]u8 };
        \\}
        \\const selected: Select() = undefined;
        \\const field = selected.<cursor>
    ...
    try testCompletion(
        ...
        \\const slice: [:9]const usize = &values;
        \\fn Select() type {
        \\    const value = slice.ptr[slice.len];
        \\    return struct { value: [value]u8 };
        \\}
        \\const selected: Select() = undefined;
        \\const field = selected.<cursor>
    ...
    try testCompletion(
        ...
        \\const slice: [:9]const usize = &values;
        \\fn Select() type {
        \\    const value = slice.ptr[slice.len + 1];
        \\    return struct { value: [value]u8 };
        \\}
        \\const selected: Select() = undefined;
        \\const field = selected.<cursor>
...
test "comptime static sentinel slice pointer identity" {
    try testCompletion(
        \\const values = [_:9]usize{ 3, 5, 7 };
        \\const slice: [:9]const usize = &values;
        \\fn Select() type {
        \\    var same: usize = 2;
        \\    if (&slice.ptr[slice.len] == &values[values.len]) same = 1;
        \\    return struct { same: [same]u8 };
        \\}
        \\const selected: Select() = undefined;
        \\const field = selected.<cursor>
    ...
    try testCompletion(
        ...
        \\fn Select() type {
        \\    var state: usize = 0;
        \\    const distance = &slice.ptr[slice.len] - &values[0];
        \\    state += 0;
        \\    return struct { distance: [distance]u8 };
        \\}
        \\const selected: Select() = undefined;
...
test "comptime static sentinel slice coercions" {
    try testCompletion(
        \\const values = [_:9]usize{ 2, 3, 5, 7 };
        \\const slice: [:9]const usize = @as([:9]const usize, &values);
        \\fn Select() type {
        \\    const value = slice[slice.len];
        \\    return struct { value: [value]u8 };
        \\}
    ...
    try testCompletion(
        \\const values = [_:9]usize{ 2, 3, 5, 7 };
        \\const range: [:7]const usize = values[1..3 :7];
        \\fn Select() type {
        \\    const value = range[range.len];
        \\    return struct { value: [value]u8 };
        \\}
        \\const selected: Select() = undefined;
        \\const field = selected.<cursor>
    ...
    try testCompletion(
        ...
        \\fn Select() type {
        \\    var same: usize = 2;
        \\    if (&range[range.len] == &values[3]) same = 1;
        \\    return struct { same: [same]u8 };
        \\}
        \\const selected: Select() = undefined;
        \\const field = selected.<cursor>
    ...
    try testCompletion(
        ...
        \\fn Select() type {
        \\    var state: usize = 0;
        \\    const distance = &range[range.len] - &values[0];
        \\    state += 0;
        \\    return struct { distance: [distance]u8 };
        \\}
        \\const selected: Select() = undefined;
...
test "comptime sentinel pointer tail boundaries" {
    try testCompletion(
        \\const values = [_:9]usize{ 2, 3, 5, 7 };
        \\const pointer: [*:9]const usize = &values;
        \\const end = pointer + values.len;
        \\fn Select() type {
        \\    var sentinel = end[0];
        \\    sentinel += 2;
    ...
    try testCompletion(
        ...
        \\fn Select() type {
        \\    var state: usize = 0;
        \\    const distance = &end[0] - &pointer[values.len];
        \\    state += 0;
        \\    return struct { distance: [distance]u8 };
        \\}
        \\const selected: Select() = undefined;
    ...
    try testCompletion(
        ...
        \\fn Select() type {
        \\    var state: usize = 0;
        \\    const distance = &pointer[values.len] - end;
        \\    state += 0;
        \\    return struct { distance: [distance]u8 };
        \\}
        \\const selected: Select() = undefined;
...
test "comptime sentinel pointer reverse boundaries" {
    try testCompletion(
        \\const values = [_:9]usize{ 2, 3, 5, 7 };
        \\const pointer: [*:9]const usize = &values;
        \\const end = pointer + values.len;
    ...
    try testCompletion(
        ...
        \\fn Select() type {
        \\    var same: usize = 2;
        \\    if (&last[0] == &values[values.len - 1]) same = 1;
        \\    return struct { same: [same]u8 };
        \\}
        \\const selected: Select() = undefined;
        \\const field = selected.<cursor>
    ...
    try testCompletion(
        ...
        \\fn Select() type {
        \\    var state: usize = 0;
        \\    const distance = &(last + 1)[0] - &values[0];
        \\    state += 0;
        \\    return struct { distance: [distance]u8 };
        \\}
        \\const selected: Select() = undefined;
...
test "comptime nested alias optional payload pointers" {
    ...
    try testCompletion(
        \\const value: ?usize = 4;
        \\fn selected(evaluations: *usize) *const ?usize {
        \\    evaluations.* += 1;
        \\    return &value;
        \\}
        \\fn Select() type {
        \\    var evaluations: usize = 0;
        \\    const pointer = if (selected(&evaluations).*) |*payload| payload else unreachable;
        \\    var same: usize = 2;
        \\    if (pointer == &value.?) same = 1;
...
test "comptime static switch pointer captures" {
    ...
    try testCompletion(
        ...
        \\fn Select() type {
        \\    var alias_same: usize = 2;
        \\    if (pointer() == &alias.count) alias_same = 1;
        \\    var original_distinct: usize = 4;
        \\    if (pointer() != &original.count) original_distinct = 3;
        \\    return struct { alias_same: [alias_same]u8, original_distinct: [original_distinct]u8 };
        \\}
        \\const selected: Select() = undefined;
        \\const field = selected.<cursor>
    , &.{
    ...
    try testCompletion(
        ...
        \\fn pointer() error{Failure}!*const usize {
        \\    return switch (try failure) {
        \\        .a => unreachable,
        \\        .b => |*payload| payload,
        \\    };
        \\}
        \\fn Select() type {
        \\    return if (pointer()) |_| struct { unexpected: u8 } else |_| struct { fallback: u8 };
        \\}
        \\const selected_value: Select() = undefined;
        \\const field = selected_value.<cursor>
    , &.{.{ .label = "fallback", .kind = .Field, .detail = "u8" }});
    ...
    try testCompletion(
        ...
        \\const first: Value = .{ .b = 4 };
        \\const second: Value = .{ .b = 2 };
        \\fn selector(evaluations: *usize) u8 {
        \\    evaluations.* += 1;
        \\    return 0;
        \\}
        \\fn pointer(evaluations: *usize) *const usize {
        \\    return switch (switch (selector(evaluations)) {
        \\        0 => first,
        \\        else => second,
        \\    }) {
        \\        .a => unreachable,
...
test "comptime static for pointer captures" {
    ...
    try testCompletion(
        \\fn values(other: bool) []const usize {
        \\    return if (other) &.{ 4, 6 } else &.{ 4, 5 };
        \\}
        \\fn pointer(other: bool) *const usize {
        \\    for (values(other)) |*item| return item;
        \\    unreachable;
        \\}
        \\fn Select() type {
        \\    var same: usize = 2;
    ...
    try testCompletion(
        \\fn values(other: bool) [3]usize {
        \\    return if (other) .{ 9, 4, 5 } else .{ 8, 4, 5 };
        \\}
        \\fn pointer(other: bool) *const usize {
        \\    for (values(other)[1..]) |*item| return item;
        \\    unreachable;
        \\}
        \\fn Select() type {
        \\    var same: usize = 2;
